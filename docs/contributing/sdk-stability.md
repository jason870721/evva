...
| `pkg/redact` | Secret redaction at the LLM egress boundary — `Redactor`, `New`, `Options`, `Rule`, `BuiltinRules`, `Finding`. Experimental for one specific reason: the **rule table is expected to grow and shift** as real credential formats change, and `Rule`'s prefilter fields (`Needs`/`NeedsOne`/`Fold`) are a performance contract that could be replaced wholesale by a different matching strategy. The `Redact`/`Findings` surface is the part least likely to move. Placeholder *format* (`[REDACTED:rule:fnv]`) is deliberately not a compatibility promise — anything parsing it is doing something the package does not support. |
