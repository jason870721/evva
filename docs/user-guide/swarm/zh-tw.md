...
- **訊息。** `send_message {to, body}`（或 `to: "all"` 廣播）寫入一條持久記錄並
  ...
  - 收信人正在**忙**（執行中）時，信件會在下一步被折進它當前的推理裡（*drain B*），
    通常幾秒內就到。
  - `send_message {to, body, urgency: "interject"}` 更進一步：它會**取消收信人當下
    正在跑的東西**——跑到一半的 build、測試套件、寫到一半的回覆——讓訊息不必等那個
    步驟結束就送達。收信人的**回合會繼續**，死掉的只有正在執行的那一步，而且它的
    對話紀錄會誠實記下自己被誰、在什麼時候中斷。

    這是用來**叫停**走錯方向的工作（「分支錯了」、「規格改了，停」），不是用來插隊。
    被取消的工作就是丟掉了，而且它已經造成的副作用**不會**被還原。只有完全相符的
    `interject` 字串才會啟動它；其他任何值都以一般方式送達——所以模型自己發明一個
    `"urgent"` 不會意外毀掉隊友的工作。
