...
memory_recall_model: ""      # 指定召回模型；留空 = 有 Anthropic 金鑰時用 Sonnet 等級，否則用當前模型
...
- **相關性召回**：每回合開始時，一次廉價的側查詢會拉入與你訊息相關的少數記憶，
  ...
  `enable_memory_recall: false` 可只保留索引而略過此額外呼叫。預設在設有 Anthropic
  金鑰時用 Sonnet 等級模型，否則用當前模型；可用 `memory_recall_model` 指定較便宜者。
