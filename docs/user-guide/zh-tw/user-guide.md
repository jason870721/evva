...
- [2. Slash 指令](#2-slash-指令)
  ...
  - [/queue — 等待送達模型的訊息](#queue--等待送達模型的訊息)
  - [內建技能](#內建技能)
- [3. 快捷鍵](#3-快捷鍵)
- [3b. 在執行中導引（steering）](#3b-在執行中導引steering)
...
在 evva 工作時打字，訊息會被送到一個你看不見的地方，而「剛剛那句送出去了嗎？」除了乾等之外沒有別的方法能回答。`/queue` 回答這個問題，並且讓你在模型讀到之前把訊息收回。
...
每一列依照模型實際會讀到的順序排列——插話在前。按 `d` 撤回游標所在的那一則。撤回**在設計上就是盡力而為**：如果代理在你看到的那一幀與你按鍵之間剛好把訊息取走了，面板會直說 `too late — already delivered`，而不是假裝成功。

這個佇列刻意**不做持久化**。evva 從未讀到的訊息，你重打一次就好；把它存起來只會在你下次還原工作階段時，讓一則早已過時的指令復活。
...
| 按鍵 | 效果 |
| --- | --- |
| `Enter` | 送出——若 evva 正在工作中，則**排入佇列**等下一個步驟（見 §3b） |
| `Ctrl+G` | **插話（interject）** — 中斷目前執行中的步驟，讓訊息立刻送達（見 §3b） |
...
evva 在工作時仍然接受輸入。你按下哪個鍵，決定訊息**何時**送達——以及代理目前的工作能保留多少。

| 你按下 | 會發生什麼 | 什麼時候用 |
| --- | --- | --- |
| `Enter` | **排入佇列。** 訊息等到目前步驟自然結束後，才以你的下一則發言送達。不會取消任何東西。 | 大多數情況。忙碌中的代理通常幾秒內就會走到下一個邊界。 |
| `Ctrl+G` | **插話。** 目前正在跑的東西——正在串流的回覆、正在執行的指令——會被取消，訊息立刻送達。**回合本身會繼續。** | 代理正在做錯的事，繼續下去只會白白浪費四分鐘。 |
| `Esc` | **中止。** 整個回合停止，什麼都不會繼續。 | 你就是要它完全停下來，接下來要做什麼由你自己決定。 |

有訊息在等待時，狀態列會顯示 `✉ 2`；其中含有插話時則顯示 `✉ 2!`。輸入框是空的時候按 `Ctrl+G` 不會有任何動作；evva 閒置時按它，行為就等同一般的 `Enter`——因為沒有東西可以中斷。

### 插話之後留下什麼

插話會**誠實交代它毀掉了什麼**，因為模型必須據此推理：

- 被取消的回覆會保留你已經看著串流出來的那段文字。它會被標記為「不完整」，而它原本準備發出的工具呼叫從未真正送出。
- 被取消的指令，其結果會寫著 `[interrupted by user before completion]`，並附上它已經產生的輸出尾段——足以看出一個 build 跑到哪裡。
- 系統註記會記下這次中斷，並且**言明已經造成的副作用不會被還原**。被砍掉的 `npm install` 會把寫到一半的 `node_modules` 原樣留在那裡。檔案需要還原請用 `/rewind`；被中斷的 shell 指令造成的其他影響，得由你自己收拾。

插話刻意用**另一個按鍵**而不是切換模式，因為被取消的呼叫等於實實在在丟掉的 token 與工作。預設是排入佇列，這是有原因的。

### 在 swarm 中插話

Swarm 成員之間也用同一套機制互相導引。`send_message` 支援 `urgency: "interject"`，會取消收件成員當下正在跑的東西。它的用途是**叫停**工作（「分支錯了」、「規格改了」），不是插隊——一般訊息本來就能在幾秒內送到忙碌的隊友手上。只有完全相符的字串才會啟動插話，其他任何值一律以一般方式送達。
