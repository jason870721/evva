...
| `pre-release` (beta) | **v1.20.0-beta.1** | cut 2026-08-04 at `162ddf4`. Carries **six** unpromoted waves: SBX (v1.15), EVAL (v1.16), CTX (v1.17), MEM (v1.18), SES (v1.19) and STE (v1.20). Per the base-version decision, a never-shipped wave takes its own claimed minor and supersedes any unpromoted older beta — so `v1.19.0-beta.1` is the last tag that base will ever carry, and v1.15–v1.19 will have no stable tags. The rows stay in the wave map regardless: they record which wave the work belongs to, not which tag carried it |
| `dev` (integration) | converged | == `pre-release` @ `v1.20.0-beta.1` |
...
**Tally: 24 stable · 6 in beta (sandbox isolation → v1.15, agent eval harness → v1.16, context engine → v1.17, memory semantic recall → v1.18, session tree → v1.19, steering v2 → v1.20 — all riding the single `v1.20.0-beta.1` tag) · 0 proposed.**
...
- ~~**`steering-v2.md`**~~ — **resolved 2026-08-04.** Built (STE-1..6), claims **v1.20**; shipped in `v1.20.0-beta.1`. W8, the last of Horizon 2. Its audit is the one to read when estimating a wave from a concept draft: the two things it planned to build *on* did not exist.
