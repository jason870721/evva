...
> **As of:** 2026-08-01, `main` @ **v1.14.0 stable**; `dev` carries four
...
| `dev` (integration) | unreleased | `main` + **SBX-1..7** (claims v1.15, merged 2026-08-01 at `023dc87`, PR #66) + **EVAL-1..7** (claims v1.16, merged 2026-08-01 at `e8f8089`, PR #67) + **CTX-1..7** (claims v1.17, merged 2026-08-01 at `c02eb8e`, PR #68) + **MEM-1..7** (claims v1.18, PR open) |

**Four unreleased waves are stacked on `dev`.** Per the base-version decision
in `CLAUDE.md`, a single `pre-release feature` cut would ship them together
and take the newest never-shipped wave's minor — **v1.18.0-beta.1**. The
v1.15–v1.17 rows stay in the wave map regardless: they record which wave the
...
## 2. Feature PRDs (`docs/roadmap/PRD/`) — 28 tracked

**Tally: 24 stable · 4 built-but-unreleased (sandbox isolation → v1.15, agent eval harness → v1.16, context engine → v1.17, memory semantic recall → v1.18) · 0 proposed.**
...
Three cases are instructive for anyone picking up a later wave. Read as a
sequence they describe an escalating failure mode: a claim that went stale, a
claim that was overtaken, and a claim that was never true.
...
code — so a wave can be wrong on arrival, not merely out of date. The audit
gate is what catches that, and MEM is the case to cite if it is ever
questioned.
...
- ~~**`memory-semantic-recall.md`**~~ — **resolved 2026-08-01.** Built (MEM-1..7), claims **v1.18**; PR open against `dev`. W6, gated on W5 by long-range §7 and picked up the moment it landed.
...
record*: waves that went through the concept → build gate (SEC, SBX, EVAL, CTX, MEM)
