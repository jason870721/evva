...
| v1.14 | Secret redaction at the LLM egress boundary (SEC-1..6) — `pkg/redact` credential detector + stable placeholders, masking at the `execTool` choke point, `/redactions` panel, `redaction` config (default ON — the one opt-OUT gate). Half of the long-range W3 "Safety" wave; its sibling SBX (sandbox isolation) stays unbuilt for want of a container runtime. PRD: `docs/roadmap/PRD/secret-redaction.md` |
