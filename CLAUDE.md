...
| v1.17 | Context engine (CTX-1..7) — block ledger + a three-rung context ladder (prune with recovery tombstones → span compaction → full compaction), `/context` overlay, pinning. **First wave of Horizon 2**; the audit pass found CTX-3 and half of CTX-5 already shipped. PRD: `docs/roadmap/PRD/context-engine.md` |
| v1.18 | Semantic memory recall (MEM-1..7) — `memory_search` tool (closes push-only recall), optional `llm.Embedder` capability + Ollama/OpenAI backends, hash-diffed vector sidecar, embedding pre-filter for the per-turn selector, `origin` provenance. Audit found the draft's premise false: recall was never index-loading, and MEM-6's "siloed" claim was inverted. PRD: `docs/roadmap/PRD/memory-semantic-recall.md` |
| v1.19 | Session tree (SES-1..7) — `evva resume` / `-c`, `evva sessions list|prune`, `evva export` (self-contained HTML, always scrubbed), `/fork` + `/title`, `/resume` grows pin/delete/all-workdirs + fork-tree indentation. Audit cut the catalog outright (a machine-wide scan measured 110 ms) and found the in-TUI picker already shipped — the gap was the pre-TUI entry. PRD: `docs/roadmap/PRD/session-tree.md` |
| v1.20 | Steering v2 (STE-1..6) — interrupt-grade mid-turn control: a per-phase cancel-with-cause seam, `Ctrl+G` interject (cancel the in-flight LLM stream or tool batch, keep the turn), partial-answer capture, paired interrupted tool results, `/queue` review-and-revoke, swarm `send_message urgency: "interject"`. **Closes Horizon 2.** PRD: `docs/roadmap/PRD/steering-v2.md` |
