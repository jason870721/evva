	...
	Create tools.Tool = tools.NewStub(
		...
		`{
			...
			"properties":{
				...
				"durable":{"type":"boolean","description":"true = persist to .claude/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this session ends."}
			}
		}`,
	)
